...
var IRCb = function (options, cb) {
  ...
  self.alternateNick = options.alternateNick;
...
IRCb.prototype._processMessage = function (message) {
  ...
  switch (command) {
    ...
    case "ERR_NICKNAMEINUSE":
      var newNick;
      if (this.nick == message.middle[1] && this.alternateNick) {
        newNick = this.alternateNick;
      } else {
        newNick = this._suffixNick(message.middle[1]);
      }
      this.nick_(newNick);
      break;
  }
...
IRCb.prototype._suffixNick = function(baseNick) {
  return baseNick + "_";
};
